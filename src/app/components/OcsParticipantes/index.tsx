...
type OCSParticipantesHomeProps = {
  id: number;
};

const OCSParticipantesHome = ({ id }: OCSParticipantesHomeProps) => {
  ...
  function responseUsersFilter(response: { users: User[] }) {
    return response.users.filter(
      ...
      (user) => user.roles?.some((role: any) => role.nome === 'agricultor'),
    );
  }
        ...
        const handleDesvincular = () => {
          if (token) {
            desvincularAgricultor(token, value)
              .then(() => {
                getUsersByOCS(token, id.toString()).then((response) =>
  ...
  const handleVincularAgricultor = () => {
    ...
    if (!token) {
      ...
      vincularAgricultorOrganizacao(token, selectedUser, id.toString())
        .then(() => {
          getUsersByOCS(token, id.toString()).then((response) =>
            setParticipants(response.users),
          );
...
export default OCSParticipantesHome;
