...
type ReunioesEditHomeProps = {
  id: number;
};

const ReunioesEditHome = ({ id }: ReunioesEditHomeProps) => {
      ...
      const requestData = {
        ...
        participantes: [
          ...
          ...(content?.participantes.map(
            (participante: { id: number }) => participante.id,
          ) ?? []),
        ],
        associacao_id: selectedAssociacoes || associacaoDefautl,
      };
      await editReuniao(requestData as Reunioes, token, id);
      router.back();
    } catch (error) {
      console.log(error);
      let errors;
...
export default ReunioesEditHome;
