...
type UsuariosEditHomeProps = {
  id: number;
};

const UsuariosEditHome = ({ id }: UsuariosEditHomeProps) => {
  ...
  const handleEditRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await editUser(requestData, token, id.toString());
      router.back();
    } catch (error: unknown) {
...
export default UsuariosEditHome;
