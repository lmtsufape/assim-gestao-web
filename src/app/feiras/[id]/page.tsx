...
const FeiraDetails = ({ params }: { params: { id: string } }) => {
  ...
  React.useEffect(() => {
    ...
    async function fetchData() {
      ...
      try {
        const feiraResponse = await getFeira(token, parseInt(params.id));
      ...
      <div className={S.container}>
        ...
        <div className={S.content}>
          ...
          {content.imagem && (
            <>
              <h3>Imagem</h3>
              <img src={content.imagem} alt={`Imagem de ${content.nome}`} />
            </>
          )}
          <div className={S.editButton}>
            <Button
              onClick={() => router.push('/feiras/editar/' + params.id)}
              type="button"
              dataType="edit"
            >
              {' '}
